...
var (
	...
	MLineLengths = stats.Int64("repl/line_lengths", "The distribution on line lengths", "By")
)
...
var (
	...
	KeyError, _  = tag.NewKey("_or")
)

// Organized metrics by creating views.
...
var (
	LatencyView = &view.View{
		...
		// Latency in buckets:
		// [>=0ms, >=25ms, >=50ms, >=75ms, >= 100ms, >=400ms, >=600ms, =800ms, >= 1s, >=2s, >=4s, >=6s]
	...
	LineLengthView = &view.View{
		...
		Measure:     MLineLengths,
		// Lengths:
		// [>=0B, >=5B, >=10B, >=15B, >=20B, >=40B, >=6-B, >=80B, >=100B, >=200B, >=400B, >=800B, >=1000B]
		Aggregation: view.Distribution(0, 5, 10, 15, 20, 40, 60, 80, 100, 20, 400, 600, 800, 1000),
	}
)
...
func processLine(ctx context.Context, in []byte) (out []byte, err error) {
	defer func(startTime time.Time) {
		stats.Record(ctx,
			MLatencyMs.M(sinceInMilliseconds(startTime)),
